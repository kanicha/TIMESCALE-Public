...
public partial class ActiveSkill
{
    ...
    /// ガードスキルの値調整処理
    /// </summary>
    private void CorrectionGuardSkill(GuardSkillBase skill, bool isJust)
    {
        // スキル時間の値保持を行う変数
        ...
        {
            // イベントの発行
            _broker.Publish(EventList.GameSystem.ActiveSetGuardSkill.GetEvent(skill, isActivatedSkillTime));
        }
        else
        {
            GuardSkillActive(skill, isActivatedSkillTime);
        }
    }
    ...
    private bool GuardSkillActiveChecker(GuardSkillBase skill, float isActivatedSkillTime, float nowTimer)
    {
        bool isActive = false;
    ...
    /// <param name="skill"> 発動しようとしているスキル </param>>
    /// <param name="isActivatedSkillTime"> スキルの発動時間調整用変数 </param>
    private void GuardSkillActive(GuardSkillBase skill, float isActivatedSkillTime)
    {
        // ジャストスキル計算用変数
        float addSkillTime = skill.skillTime + isActivatedSkillTime;
