...
public partial class ActiveSkill
{
    ...
    /// 攻撃スキルの値調整処理
    /// </summary>
    private void CorrectionAssistSkill(AssistSkillBase assistSkill, bool isJust)
    {
        // スキル時間の値保持を行う変数
        ...
        {
            // イベントの発行
            _broker.Publish(EventList.GameSystem.ActiveSetAssistSkill.GetEvent(assistSkill, isActivatedSkillTime));
        }
        else
        {
            AssistSkillActive(assistSkill, isActivatedSkillTime);
        }
    }
    ...
    private bool AssistSkillActiveChecker(AssistSkillBase skill, float isActivatedSkillTime, float nowTimer)
    {
        bool isActive = false;
    ...
    /// <param name="skill"> 発動しようとしているスキル </param>>
    /// <param name="isActivatedSkillTime"> スキルの発動時間調整用変数 </param>
    private void AssistSkillActive(AssistSkillBase skill, float isActivatedSkillTime)
    {
        // ジャストスキル計算用変数
        float addSkillTime = skill.skillTime + isActivatedSkillTime;
